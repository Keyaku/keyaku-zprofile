...
if (( ! ${#ZPROFILE_MODULES} )); then
	if [[ ! -s "$ZSH_CACHE_HOME/zprofile_modules" || "$ZDOTDIR/.gitmodules" -nt "$ZSH_CACHE_HOME/zprofile_modules" ]]; then
		...
		print -l "${ZPROFILE_MODULES}" > "$ZSH_CACHE_HOME/zprofile_modules"
	else
